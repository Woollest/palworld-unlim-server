...
PalOps右上の「アプリをインストール」を選択します。Microsoft EdgeまたはGoogle Chromeでインストールすると、スタートメニューやタスクバーからアドレスバーのない独立ウィンドウとして起動できます。ローカルAPIはキャッシュせず、状態と操作結果は常に稼働中のPalOpsから取得します。
