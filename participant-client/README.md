...
初回導入時だけ、Unlim公式手順に従って `%LOCALAPPDATA%\Unlim` をWindows Defenderの除外へ追加するため、UACの管理者確認を表示します。許可されたことを確認してから、公式版の取得、サイズ・SHA-256検証、配置、ユーザーPATH登録、`unlim --version`による実行確認を行います。通常の更新と接続では管理者権限を要求しません。
