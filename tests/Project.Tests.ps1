Describe 'Palworld Server repository' {
    ...
    It 'keeps the local participant app thin, verified and port-agnostic' {
        ...
        if ($Manager -notmatch 'await using \(var stream' -or $Manager -match 'await using var stream = File\.OpenRead\(staging\)') { throw 'Participant app keeps its verification stream open while replacing Unlim.' }
